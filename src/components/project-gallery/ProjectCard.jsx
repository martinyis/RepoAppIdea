...
const ProjectCard = (props) => {
  ...
  const fixedColors = ["#D4ADFC", "#1D267D", "#0C134F", "#5C469C"];
  ...
  return (
    ...
    >
      ...
      <div
        id="techStack"
        className="h-14 bg-gray-100 rounded-t-lg rounded-b-3xl overflow-hidden"
      >
        <div className="scrollable-container overflow-x-auto py-2">
          <div className="scrollable-content whitespace-nowrap flex space-x-4">
            {data.techStack.map((el, index) => (
              <div className="tech-item text-white px-4 py-2 rounded-full text-sm" style={{ backgroundColor: fixedColors[index % fixedColors.length] }}>
                {el}
              </div>
            ))}
          </div>
